...
type deviceAuthenticate interface {
	...
	// GetDeviceAuthorizationByUserCode resturns the current state of the device
	// authorization flow,
	...
	// CompleteDeviceAuthorization marks a device authorization entry as
	// Completed,
...
func (d *deviceLogin) loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	state, err := d.storage.GetDeviceAuthorizationByUserCode(
		r.Context(),
		userCode,
	)
	if err != nil {
	...
	encoded, err := d.cookie.Encode(
		userCodeCookieName,
		userCodeCookie{userCode, username},
	)
	if err != nil {
...
func (d *deviceLogin) confirmHandler(w http.ResponseWriter, r *http.Request) {
	...
	switch action {
	case "allowed":
		err = d.storage.CompleteDeviceAuthorization(
			r.Context(),
			data.UserCode,
			data.UserName,
		)
	...
	fmt.Fprintf(
		w,
		"Device authorization %s. You can now return to the device",
		action,
	)
}
