...
// customClaim demonstrates how to return custom claims based on provided
// information
...
func (s *Storage) StoreDeviceAuthorization(
	ctx context.Context,
...
func (s *Storage) GetDeviceAuthorizatonState(
	ctx context.Context,
	...
	if !ok || entry.state.ClientID != clientID {
		return nil, errors.New(
			"device code not found for client",
		) // is there a standard not found error in the framework?
	}

	return entry.state, nil
}

func (s *Storage) GetDeviceAuthorizationByUserCode(
	ctx context.Context,
	userCode string,
) (*op.DeviceAuthorizationState, error) {
	s.lock.Lock()
...
func (s *Storage) CompleteDeviceAuthorization(
	ctx context.Context,
	userCode, subject string,
) error {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Storage) DenyDeviceAuthorization(
	ctx context.Context,
	userCode string,
) error {
	s.lock.Lock()
...
// AuthRequestDone is used by testing and is not required to implement
// op.Storage
...
func (s *Storage) ClientCredentials(
	ctx context.Context,
	clientID, clientSecret string,
) (op.Client, error) {
	s.lock.Lock()
...
func (s *Storage) ClientCredentialsTokenRequest(
	ctx context.Context,
	clientID string,
	scopes []string,
) (op.TokenRequest, error) {
	client, ok := s.serviceUsers[clientID]
	if !ok {
		return nil, errors.New("wrong service user or password")
	}
